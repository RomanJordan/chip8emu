...
#define RESERVED_BLOCK 512 // 0x200 in memory
#define BITS_ROW 8
...
bool cpu::loadProgram(const char* mROM)
{
	FILE* pROM;
	errno_t err;
	if ((err = fopen_s(&pROM, mROM, "rb")) != 0) {
		std::cout << "Couldn not open the program";
		return false;
	}

	// Get the size of the ROM
	int size = getFileSize(mROM);
	if (size == -1) // Check if the ROM exists
	{
		std::cout << "Could not open the program";
		return false;
	}
	else if ((4096 - RESERVED_BLOCK) < size) {
	...
	for (int i = 0; i < size; i++)
		memory[RESERVED_BLOCK + i] = buffer[i];
...
void cpu::emulateCycle()
{
		...
		for (int yLine = 0; yLine < height; yLine++) // Loop over each row
		{
			...
			for (int xLine = 0; xLine < BITS_ROW; xLine++) // Loop over 8 bits of one row
			{
				if ((pixel & (0x80 >> xLine)) != 0) // Check if the current evaluated pixel is set to 1 (note that 0x80 >> xline scan through the byte, one bit at the time)
				{
					...
					if (display[pixDisp] == 1) // Check if the pixel on the display is set to 1. If it is set, we need to register the collision by setting the VF register
						V[0xF] = 1;
					display[pixDisp] ^= 1; // Set the pixel value by using XOR
				}
			}
		}
	...
	if (sound_timer > 0)
	{
		if (sound_timer != 0)
			std::cout << "BEEP!\n";
		//Beep(523, 75); // This beep sound is incredibly annoying & loud
		sound_timer--;
	}
}